...
pub async fn run_panel(
    ...
    // Background Tunnel End-to-End Health & Traffic Prober
    let db_path_tunnel_probe = db_path.clone();
    tokio::spawn(async move {
        let mut interval = tokio::time::interval(std::time::Duration::from_secs(30));
        loop {
            interval.tick().await;
            if let Ok(tunnels) = db::get_tunnels(&db_path_tunnel_probe) {
                for mut tunnel in tunnels {
                    if tunnel.status == "inactive" {
                        continue;
                    }
                    let t_id = match tunnel.id {
                        Some(id) => id,
                        None => continue,
                    };
                    
                    let target_ip = if let Some(i_id) = tunnel.iran_node_id {
                        if let Ok(Some(node)) = db::get_node_by_id(&db_path_tunnel_probe, i_id) {
                            node.host
                        } else {
                            "127.0.0.1".to_string()
                        }
                    ...
                    let target_port = tunnel.iran_port;
                    ...
                    match probe_res {
                        Ok(Ok(mut stream)) => {
                            ...
                            let probe_payload = b"CHERAGH_E2E_PING_PROBE_V1.18.0\n";
                        ...
                        _ => {
                            ...
                            if let Some(k_id) = tunnel.kharej_node_id {
                                if let Ok(Some(_k_node)) = db::get_node_by_id(&db_path_tunnel_probe, k_id) {
                                    if let Ok(nodes) = db::get_nodes(&db_path_tunnel_probe) {
                                        let backup_nodes: Vec<_> = nodes
                                            .into_iter()
                                            .filter(|n| n.id != Some(k_id) && n.status.as_deref() == Some("active") && (n.role == "kharej" || n.role == "both"))
                                            .collect();
                                        if let Some(backup) = backup_nodes.first() {
                                            println!("[E2E PROBE FAILOVER] Tunnel '{}' (ID {}) probe failed. Failing over to backup Node '{}'", tunnel.name, t_id, backup.name);
                                            tunnel.kharej_node_id = backup.id;
                                            let _ = db::update_tunnel(&db_path_tunnel_probe, t_id, &tunnel);
                                        }
                                    }
                                }
                            }
